...
import AddNewItemModal from "../../../components/page/thuy-loi/kenh-muong/modalAddNew";
import "bootstrap/dist/css/bootstrap.min.css";
import { Modal, ModalBody, ModalFooter, ModalHeader } from "reactstrap";
import ModalEdit from "../../../components/page/thuy-san/quan-ly-tau-ca/modalEdit";
import kenhMuongServices from "~/services/kenhMuongServices"; // Import kenhMuongServices
...
export default function Page() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await kenhMuongServices.display(data); // Use kenhMuongServices to fetch data
  ...
  const handleAdd = async () => {
    try {
      const response = await kenhMuongServices.create(newItem); // Use kenhMuongServices to create a new item
  ...
  const handleUpdate = async (editedItem: any) => {
    try {
      const response = await kenhMuongServices.update(
        editedItem.id,
        editedItem
      ); // Use kenhMuongServices to update an item
  ...
  const handleConfirmDelete = async (deleteItem: any) => {
    try {
      const response = await kenhMuongServices.delete(deleteItem.id); // Use kenhMuongServices to delete an item
  ...
  return (
    <>
      ...
      >
        <Head>
          <title>{i18n.t("Quản lý kênh mương")}</title>
        </Head>
